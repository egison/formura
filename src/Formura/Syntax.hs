...
data OperatorF x = UniopF IdentName x
...
typeDecls (BindingF stmts0) = concat $ flip map stmts0 $ \case
  TypeDeclF (ModifiedTypeExpr _ t) l | t == TopType -> []
                                     | otherwise -> [(l,t)]
  _ -> []

-- | extract all 'TypeModifier's from the binding
typeModifiers :: BindingF x -> [(LExpr, TypeModifier)]
typeModifiers (BindingF stmts0) = concat $ flip map stmts0 $ \case
  TypeDeclF (ModifiedTypeExpr ms _) l -> [(l,m) | m <- ms]
  _ -> []

-- | extract all the 'Subst'itutions from the binding
substs :: BindingF x -> [(LExpr, x)]
substs (BindingF stmts0) = concat $ flip map stmts0 $ \case
  SubstF l r -> [(l, r)]
  _ -> []


-- | Statement
data StatementF x = SubstF LExpr x
                  -- ^ substitution
